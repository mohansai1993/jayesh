...
const SelectSearchConsoleDomain = ({ tokenKey, gscAccessToken }) => {
  ...
  const loadSearchConsoleDomainsList = async () => {
    ...
    try {
      const res =
        gscAccessToken &&
        (await dispatch(getSearchConsoleDomainsList(gscAccessToken as string)));
      if (res?.success) {
        setLoading(false);
      }
      if (res?.error) {
  ...
  const searchConsoleDomainHandler = (siteUrl) => {
        ...
        <Card style={{ minWidth: "30%" }}>
          ...
          <Card.Body className="select-account">
            ...
            {searchConsoleDomainsList &&
              searchConsoleDomainsList.map((siteUrl, index) => {
                return (
                  <div key={index} className="select-account">
                    <div key={index} className="select-account__child">
                      <>
                        {" "}
                        {siteUrl.replace(/.*:/, "")}
                        <div
                          onClick={() => searchConsoleDomainHandler(siteUrl)}
