...
const SelectFbInsightsPage = ({ userToken, tokenKey }) => {
  ...
  const loadFbInsightsPagesList = async () => {
    ...
    try {
      const res =
        userToken &&
        (await dispatch(getFbInsightsPagesList(userToken as string)));
      if (res?.success) {
        setLoading(false);
      }
      if (res?.error) {
  ...
  const fbPageHandler = async (id, info) => {
      ...
      <div className="card-outer">
        <Card style={{ minWidth: "30%" }}>
          ...
          <Card.Body className="select-account">
            ...
            {fbInsightsPagesList &&
              Object.entries(fbInsightsPagesList)?.map(([pageId, pageInfo]) => {
                return (
                  <div key={pageId} className="select-account">
                    <div key={pageId} className="select-account__child">
                      <>
                        {pageInfo?.["name"]}
