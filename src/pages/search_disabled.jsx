...
const SearchPage = ({ tokenKeyFromCookie }) => {
  ...
  const [subscriptionPaymentMonthly, setSubscriptionPaymentMonthly] =
    useState(true);
    ...
    async (e) => {
      ...
      try {
        ...
        if (filterColumnIndex >= 0) {
          const filteredResult = result?.result?.google_ads?.map(
            (adArray) => {
    ...
    >
      <div className="search">
        <div className="row m-0 h-100 w-100 px-4">
          ...
          <div
            className={`col-lg-10 col-md-10 col-12 search__right search__result ${
              barWidth == "1" ? "mt-4" : "m-lg-auto m-md-auto"
            }`}
          >
            <div
