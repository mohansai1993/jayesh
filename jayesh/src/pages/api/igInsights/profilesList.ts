...
export default async (req: NextApiRequest, res: NextApiResponse) => {
    if (req.method === 'GET') {
        ...
        try {
            ...
            const copterRes = await axios.get(`${API_URL}/ig_profiles_list?token_key=${token_key}`, config);
            if (copterRes?.status === 200) {
                return res?.status(200)?.json(copterRes?.data);
            }
            return res?.status(copterRes?.status)?.json({
                error: "Failed to get IG Profiles list",
            });
        } catch (error) {
            // To log any errors if the account hierarchy fails to load
            console.log(error);
            return res?.status(500)?.json({
                error: "An error occurred in getting IG profiles",
            });
        }
    }
